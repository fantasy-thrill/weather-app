import { ForecastObject, Hour3ForecastItem, WeatherItem } from "./interfaces.js";

export function capitalizeName(city: string): string {
...
export function displayIcon(obj: WeatherItem): string {
  const period: string = obj.icon[2]
...
export function degreesToCardinal(degrees: number): string {
 ...
 export function getBackgroundColor(data: ForecastObject): string {
...
export function setWeatherBackground(data: ForecastObject) {
...
export function getTime(timestamp: number, timeZone: string, twentyFourHour: boolean = true) {
...
export function getDayOfWeek(timestamp: number, length: string) {
  const dateObject = new Date(timestamp * 1000)
...
export function uvIndexFormat(index: number) {
...
export function isTimeWithinFrame(current: string, startHour: number, endHour: number) {
...
export function buildTwelveHour(objArr: Hour3ForecastItem[], startHour: number, endHour: number, timeZone: string): Hour3ForecastItem[] {
  const forecastArr = []

  const equalHour = (obj: Hour3ForecastItem, num: number, carryOver: boolean = false) => {
  ...
  const firstForecast = objArr.find((hour: Hour3ForecastItem) => equalHour(hour, startHour))
  const secondForecast = objArr.find((hour: Hour3ForecastItem) => equalHour(hour, endHour, true))

  if (firstForecast) forecastArr.push(firstForecast)
  if (secondForecast) forecastArr.push(secondForecast)
  return forecastArr
}
